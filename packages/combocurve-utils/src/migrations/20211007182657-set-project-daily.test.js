...
// eslint-disable-next-line @typescript-eslint/no-var-requires -- TODO eslint fix later
const { up } = require('./20211007182657-set-project-daily');
...
describe('20211007182657-set-project-daily', () => {
	let dailyProdCollection;

	beforeAll(async () => {
		dailyProdCollection = db.collection('daily-productions');
		await db.collection('wells').insertMany(wells);
		await dailyProdCollection.insertMany(dailyProdDataBefore);
	});

	test('up', async () => {
		await up({ db });

		const result = await dailyProdCollection.find({}).sort({ _id: 1 }).toArray();

		expect(result).toEqual(dailyProdDataAfter);
	});
