...
from combocurve.science.segment_models.shared.helper import pred_arps, pred_exp, exp_D_eff_2_D, arps_get_D_delta
from combocurve.science.forecast_models.shared.segment_shared import arps_modified_para2seg, exp_para_2_seg


class model_exp_dec_arps_modified_free_peak_same(model_parent):
    def __init__(self):
        self.model_name = 'exp_dec_arps_modified_free_peak_same'
        self.model_p_name = ['D_eff', 'q_peak', 'minus_t1_t_peak', 'b']
    ...
    def func(self, t, p, p_fixed):
        [D_eff, q_peak, minus_t1_t_peak, b] = p
        [t_first, minus_t_peak_t_first] = p_fixed

        t_peak = t_first + minus_t_peak_t_first
        t1 = t_peak + minus_t1_t_peak
        D_exp = exp_D_eff_2_D(D_eff)
        D_arps = D_exp  #based on the calculate of first derivative
    ...
    def p2seg(self, p, p_fixed, p2seg_dict):
        ...
        t_peak = t_first + minus_t_peak_t_first
        t1 = t_peak + minus_t1_t_peak
        q1 = pred_exp(t1, t_peak, q_peak, D)

        t1_end = int(np.floor(t1))
        q1_end = pred_exp(t1_end, t_peak, q_peak, D)
        t2_start = t1_end + 1
        q2_start = pred_arps(t2_start, t1, q1, D, b)

        D_arps_t2_start = arps_get_D_delta(D, b, t2_start - t1)

        first_seg = exp_para_2_seg(t1_end, t1_end, q1_end, D_eff, t_peak, q_peak)
