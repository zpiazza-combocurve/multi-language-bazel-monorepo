...
from combocurve.science.forecast_models.shared.deterministic_shared import det_pred_segment_arps_inc_arps_4
...
class model_segment_arps_inc_arps_4_free_peak_free_b1(model_segment_arps_4_parent):
    def __init__(self):
        self.model_name = 'segment_arps_inc_arps_4_free_peak_free_b1'
        self.model_p_name = ['q0', 'D1_eff', 'b1', 'minus_t_elf_t_peak', 'b2', 'b0', 'q_peak']
        self.model_p_fixed_name = ['t_first', 'minus_t0_t_first', 'minus_t_peak_t0']
        ...
        self.TC_c8_para_name = ['b0', 'q_peak', 'b1', 'D1_eff', 'b2']
    ...
    def func(self, t, p, p_fixed):
        [q0, D1_eff, b1, minus_t_elf_t_peak, b2, b0, q_peak] = p
        ...
        return det_pred_segment_arps_inc_arps_4(t, t0, q0, b0, t_peak, q_peak, b1, D1, t_elf, b2, D2)
    ...
    def prepare(self, data, t_peak, transformation, p2seg_dict=None):
        ...
        p_fixed = np.array([t_first, t0 - t_first, t_peak - t0])
        ...
        p_range = [
            ...
            (1e-5, 1 - 1e-5),
            (1e-5, 2),
            (0, np.max(transformed_data[:, 0]) - t_peak),
            (1e-5, 2),
            (-10, -1e-5),
    ...
    def p2seg(self, p, p_fixed, p2seg_dict):
        ...
        [q0, D1_eff, b1, minus_t_elf_t_peak, b2, b0, q_peak] = p
        [start_idx, minus_t0_t_start, minus_t_peak_t0] = p_fixed
        ...
        return segment_arps_4_paras2seg(start_idx, t_end_life, q_final, D_lim_eff, end_data_idx, life_idx, enforce_sw,
                                        t0, q0, t_peak, q_peak, D1_eff, b1, t1_end, D2, b2, t2_start, q2_start, b0,
                                        False)

    def TC_set_before_peak(self, p, p_fixed):
        params = self.get_parameters_as_dict(p, p_fixed)

        params['D1_eff'] = 0.99
        params['minus_t_elf_t_peak'] = 60
        params['b1'] = 1.1
        params['b2'] = 1.1

        ret_p, ret_p_fixed = self.pdict_to_p_and_p_fixed(params)
        return ret_p, ret_p_fixed

    def TC_set_after_peak(self, p, p_fixed):
        return self.segment_arps_4_wp_set_after_peak(p, p_fixed, False)

    def TC_buildup(self, p, p_fixed, t_peak, buildup_dict, raito_in_cum_fit=False):
        return self.segment_arps_4_wp_buildup(p, p_fixed, t_peak, buildup_dict, raito_in_cum_fit, False)
