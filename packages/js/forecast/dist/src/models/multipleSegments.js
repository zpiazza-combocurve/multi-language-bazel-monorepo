...
const arpsInc_1 = __importDefault(require("./arpsInc"));
const arpsModified_1 = __importDefault(require("./arpsModified"));
    ...
    cumFromT({ idxArr, production, series = [], phase, dataFreq }) {
        ...
        if (origProdArr?.length && origIndex?.length) {
            prodArr =
                dataFreq === 'monthly'
                    ? origProdArr.map((v) => {
                        return v * helper_1.DAYS_IN_MONTH;
                    })
                    : origProdArr;
            index = origIndex;
        }
        ...
        if (index?.length) {
            ...
            if (dataFreq === 'daily') {
                ...
                dataEndIdx = (0, helper_1.getLastDayOfMonth)(lastMonth);
            }
        }
        ...
        idxArr.forEach((t) => {
            if (t < dataStartIdx) {
                ...
                if (dataFreq === 'daily') {
                    ...
                    let lastDayOfMonth = (0, helper_1.getLastDayOfMonth)(index[currentIndex]);
                    while (lastDayOfMonth < t) {
                        prevSum += prodArr[currentIndex++];
                        lastDayOfMonth = (0, helper_1.getLastDayOfMonth)(index[currentIndex]);
                    }
