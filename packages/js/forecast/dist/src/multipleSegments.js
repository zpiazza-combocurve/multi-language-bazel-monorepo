...
const arps_1 = __importDefault(require("./models/arps"));
const arpsInc_1 = __importDefault(require("./models/arpsInc"));
const arpsModified_1 = __importDefault(require("./models/arpsModified"));
const empty_1 = __importDefault(require("./models/empty"));
const expDec_1 = __importDefault(require("./models/expDec"));
const expInc_1 = __importDefault(require("./models/expInc"));
const flat_1 = __importDefault(require("./models/flat"));
const linear_1 = __importDefault(require("./models/linear"));
class MultipleSegments {
    constructor(segments = [], relativeTime = false) {
        this.segmentClassDict = {
            exp_inc: expInc_1.default,
            exp_dec: expDec_1.default,
            arps: arps_1.default,
            arps_inc: arpsInc_1.default,
            arps_modified: arpsModified_1.default,
            flat: flat_1.default,
    ...
    shiftSegmentsIdx({ inputSegments, deltaT, shiftStartIdx = 0, goBackward = false, inputAsObject = false, outputAsObject = false, relativeTime = false, }) {
        let sliceStartIndex;
        let sliceEndIndex;
        if (goBackward) {
            sliceStartIndex = 0;
    ...
    predictSelf(idxArr, toFill = null) {
        ...
        const trailingOutOfRangeIdxArr = idxArr.filter((idx) => idx > this.segmentObjects[this.segmentObjects.length - 1].segment.end_idx);
        ret = ret.concat(Array(trailingOutOfRangeIdxArr.length).fill(toFill));
        return ret;
    }
    ...
    rateEur({ cumData, endDataIdx, leftIdx, rightIdx, forecastSegments, dataFreq = 'monthly', relativeTime = false }) {
        ...
        if (dataFreq === 'monthly') {
            const endDataDate = (0, math_1.convertIdxToDate)(endDataIdx);
            const endDataMonthEndDate = new Date(endDataDate.getFullYear(), endDataDate.getMonth() + 1, 0);
            useEndDataIdx = (0, math_1.convertDateToIdx)(endDataMonthEndDate);
        }
    ...
    ratioEurInterval({ cumData, endDataIdx, leftIdx, rightIdx, ratioTSegments, baseSegments, dataFreq = 'monthly' }) {
        ...
        if (!Array.isArray(ratioTSegments) ||
        ...
        const useRightIdx = Math.min(rightIdx, ratioTSegments[ratioTSegments.length - 1].end_idx, baseSegments[baseSegments.length - 1].end_idx);
        ...
        return (pred.slice(0, rightStopIndex).reduce((acc, v) => acc + v * 30, cumData) +
    ...
    cumFromT({ idxArr, production, series = [], phase, dataFreq }) {
        ...
        if (origProdArr?.length && origIndex?.length) {
            prodArr =
                dataFreq === 'monthly'
                    ? origProdArr.map((v) => {
                        return v * helpers_1.DAYS_IN_MONTH;
                    })
                    : origProdArr;
            index = origIndex;
        }
        ...
        if (index?.length) {
            ...
            if (dataFreq === 'daily') {
                ...
                dataEndIdx = (0, helpers_1.getLastDayOfMonth)(lastMonth);
            }
        }
        ...
        idxArr.forEach((t) => {
            if (t < dataStartIdx) {
                ...
                if (dataFreq === 'daily') {
                    ...
                    let lastDayOfMonth = (0, helpers_1.getLastDayOfMonth)(index[currentIndex]);
                    while (lastDayOfMonth < t) {
                        prevSum += prodArr[currentIndex++];
                        lastDayOfMonth = (0, helpers_1.getLastDayOfMonth)(index[currentIndex]);
                    }
                    // currentIndex will be exactly the month that has end_month_idx >= t
                    const curIndexMonthStartIdx = index[currentIndex] - 14;
                    const tDate = (0, math_1.convertIdxToDate)(t);
                    const lastMonthDate = (0, math_1.convertIdxToDate)(lastDayOfMonth);
                    ...
                    if (tDate.getFullYear() === lastMonthDate.getFullYear() &&
...
exports.default = MultipleSegments;
