...
import ExpDecSegment from './expDec';

describe('SegmentModels-ExpDecSegment', () => {
	const arpsSegment = new ArpsSegment({
		end_idx: 43367,
		name: 'arps',
		start_idx: 41542,
		b: 0.9,
	...
	const expDecSegment0 = new ExpDecSegment({
		end_idx: 43367,
		name: 'exp_dec',
		start_idx: 41542,
	...
	const expDecSegment = new ExpDecSegment({
		end_idx: 45193,
		name: 'exp_dec',
		start_idx: 43368,
	...
	test('ExpDecSegment-changeQStart', () => {
		...
		expect(expDecSegment.changeQStart(2000).q_end).toBeCloseTo(
			expDecSegment.segment.q_end * (2000 / expDecSegment.segment.q_start)
		);
	});
	...
	test('ExpDecSegment-changeDuration', () => {
		...
		expect(expDecSegment.changeDuration(44327 - expDecSegment.segment.start_idx + 1).q_end).toBeCloseTo(
			0.18182030595876492
		);
	});
	...
	test('ExpDecSegment-buttonConnectPrev', () => {
		...
		expect(expDecSegment.buttonConnectPrev(arpsSegment.segment).q_end).toBeCloseTo(
			(expDecSegment.segment.q_end * arpsSegment.segment.q_end) / expDecSegment.segment.q_start
		);
	});
	...
	test('ExpDecSegment-buttonConnectNext', () => {
		...
		expect(expDecSegment0.buttonConnectNext(expDecSegment.segment).q_start).toBeCloseTo(
			(expDecSegment0.segment.q_start * expDecSegment.segment.q_start) / expDecSegment0.segment.q_end
		);
	});
	...
	test('ExpDecSegment-calcQStart', () => {
		expect(
			expDecSegment0.calcQStart({ start_idx: 41542, end_idx: 43367, q_end: 11.620024730691494, D_eff: 0.35 })
				.q_start
		).toBeCloseTo(100);
		expect(
			expDecSegment.calcQStart({ start_idx: 43368, end_idx: 45193, q_end: 0.010469066869520068, D_eff: 0.7 })
				.q_start
		).toBeCloseTo(4.290542865400525);
	});

	test('ExpDecSegment-calcEndIdx', () => {
		expect(
			expDecSegment0.calcEndIdx({ start_idx: 41542, q_start: 100, q_end: 11.620024730691494, D_eff: 0.35 })
				.end_idx
		).toBeCloseTo(43367);
		expect(
			expDecSegment.calcEndIdx({
				start_idx: 43368,
				q_start: 4.290542865400525,
				q_end: 0.010469066869520068,
				D_eff: 0.7,
	...
	test('ExpDecSegment-calcQEnd', () => {
		expect(
			expDecSegment0.calcQEnd({ start_idx: 41542, end_idx: 43367, q_start: 100, D_eff: 0.35 }).q_end
		).toBeCloseTo(11.620024730691494);
		expect(
			expDecSegment.calcQEnd({ start_idx: 43368, end_idx: 45193, q_start: 4.290542865400525, D_eff: 0.7 }).q_end
		).toBeCloseTo(0.010469066869520068);
	});

	test('ExpDecSegment-calcDeff', () => {
		expect(
			expDecSegment0.calcDeff({ start_idx: 41542, end_idx: 43367, q_start: 100, q_end: 11.620024730691494 }).D_eff
		).toBeCloseTo(0.35);
		expect(
			expDecSegment.calcDeff({
				start_idx: 43368,
				end_idx: 45193,
				q_start: 4.290542865400525,
				q_end: 0.010469066869520068,
