import { DAYS_IN_YEAR, DEFAULT_MAX_D_EFF } from '../helpers/helper';
import { ceilFloat, convertDateToIdx, convertIdxToDate, fixedFloat, floorFloat } from '../helpers/math';
...
const convertDEffToCalc = (value) => Math.min(value / 100, DEFAULT_MAX_D_EFF);

const retSelf = (x) => x;
export default class SegmentParent {
	constructor(segment, relativeTime = false) {
		this.segment = segment;
		// Upper bound at 1,000,000,000,000 for essentially unlimited headroom without running into numerical issues.
	...
	calcToView({ segment = this.segment, unitConvertFunc = null, idxDate = false } = {}) {
		...
		const idxConvert = idxDate ? retSelf : convertIdxToDate;
		let ret = {
			...segment,
		...
		Object.entries(ret).forEach(([key, value]) => {
			if (VIEW_FIXED_DECIMAL_KEYS.includes(key)) {
				ret[key] = fixedFloat(value, VIEW_FIXED_DECIMAL_MAP[key]);
			}
		});
		return ret;
	}
	...
	getCalcRange({ segment = this.segment } = {}) {
		...
		if (segment.name === 'linear') {
			...
			return {
				...
				D_eff: [-Infinity, DAYS_IN_YEAR / (segment.end_idx - segment.start_idx)],
			};
		}
	...
	getFormViewRange({ segment = this.segment, unitConvertFunc = null, idxDate = false, toBeCalculatedParam }) {
		...
		const calcLower = Object.entries(calcRange).reduce(
			(acc, [k, v]) => ({
				...acc,
				[k]: v[0],
			}),
		...
		const calcUpper = Object.entries(calcRange).reduce(
			(acc, [k, v]) => ({
				...acc,
				[k]: v[1],
			}),
		...
		return Object.entries(calcRange).reduce(
			(acc, [k]) => ({
				...acc,
	...
	viewToCalc({ viewSegment, unitConvertFunc = null, idxDate = false }) {
		const unitConvert = unitConvertFunc ?? retSelf;
		const idxConvert = idxDate ? retSelf : convertDateToIdx;
		let ret = {
			...viewSegment,
	...
	getViewRange({ segment = this.segment, unitConvertFunc = null, idxDate = false } = {}) {
		...
		switch (segment.name) {
			...
			case 'linear': {
				...
				ret = {
					...
					D_eff: [-Infinity, (DAYS_IN_YEAR / (segment.end_idx - segment.start_idx)) * 100],
				};
				break;
			}
			default: {
		...
		Object.entries(ret).forEach(([key, value]) => {
			if (VIEW_FIXED_DECIMAL_KEYS.includes(key)) {
				ret[key][0] = ceilFloat(value[0], VIEW_FIXED_DECIMAL_MAP[key]);
				ret[key][1] = floorFloat(value[1], VIEW_FIXED_DECIMAL_MAP[key]);
			}
		});
