import LinearSegment from './linear';

describe('SegmentModels-LinearSegment', () => {
	const linearSegment1 = new LinearSegment({
		end_idx: 41541,
		name: 'linear',
		start_idx: 39716,
	...
	const linearSegment2 = new LinearSegment({
		end_idx: 43367,
		name: 'linear',
		start_idx: 41542,
	...
	test('LinearSegment-changeQStart', () => {
		...
		expect(linearSegment1.changeQStart(2000).q_end).toBeCloseTo(
			linearSegment1.segment.q_end + (2000 - linearSegment1.segment.q_start)
		);
	});
	...
	test('LinearSegment-changeDuration', () => {
		...
		expect(linearSegment1.changeDuration(44327 - linearSegment1.segment.start_idx + 1).q_end).toBeCloseTo(
			1763.2876712328766
		);
	});
	...
	test('LinearSegment-buttonConnectPrev', () => {
		expect(linearSegment2.buttonConnectPrev(linearSegment1.segment).q_start).toBeCloseTo(
			linearSegment1.segment.q_end
		);
		expect(linearSegment2.buttonConnectPrev(linearSegment1.segment).q_end).toBeCloseTo(
			linearSegment2.segment.q_end + linearSegment1.segment.q_end - linearSegment2.segment.q_start
		);
	});
	...
	test('LinearSegment-buttonConnectNext', () => {
		...
		expect(linearSegment1.buttonConnectNext(linearSegment2.segment).q_start).toBeCloseTo(
			linearSegment1.segment.q_start + linearSegment2.segment.q_start - linearSegment1.segment.q_end
		);
	});
	...
	test('LinearSegment-calcK', () => {
		expect(
			linearSegment1.calcK({
				start_idx: 39716,
				end_idx: 41541,
				q_start: 500,
				q_end: 1000,
			}).k
		).toBeCloseTo(0.273972602739726);
		expect(
			linearSegment2.calcK({
				start_idx: 41542,
				end_idx: 43367,
				q_start: 2000,
				q_end: 1000,
			}).k
		).toBeCloseTo(-0.547945205479452);
	});
});
