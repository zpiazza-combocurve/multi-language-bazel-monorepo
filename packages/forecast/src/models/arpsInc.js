...
export default class ArpsIncSegment extends ArpsSegment {
		...
		switch (toBeCalculatedParam) {
			case 'end_idx': {
				if (segment.start_idx === segment.end_idx) {
					...
					DeffMin = Math.min(
						...
						Math.max(
							-this.numericLarge,
							arpsD2Deff(arpsGetD({ ...segment, end_idx: segment.start_idx + 1 }), segment.b)
						)
					);
					DeffMax = Math.max(
						-this.numericLarge,
						Math.min(-0.01, arpsD2Deff(arpsGetD({ ...segment, end_idx: this.dateIdxLarge }), segment.b))
					);
				}
				return {
					start_idx: [
					...
					q_start: [
						Math.min(
							segment.q_end,
							Math.max(this.numericSmall, arpsGetQStart({ ...segment, D, end_idx: this.dateIdxLarge }))
						),
						segment.q_end,
					],
					q_end: [
						...
						Math.max(
							segment.q_start,
							Math.min(this.numericLarge, predArps({ ...segment, t: this.dateIdxLarge, D }))
						),
					],
			...
			case 'q_start': {
				DeffMin =
					segment.start_idx === segment.end_idx
						...
						: Math.min(
								...
								Math.max(
									-this.numericLarge,
									arpsD2Deff(arpsGetD({ ...segment, q_start: this.numericSmall }), segment.b)
								)
						  );
				startIdxMin = Math.min(
					...
					Math.max(
						this.dateIdxSmall,
						arpsGetStartIdxFromQstart({ ...segment, q_start: this.numericSmall, D })
					)
				);
				...
				endIdxMax = Math.max(
					segment.start_idx,
					Math.min(this.dateIdxLarge, arpsGetEndIdxFromQend({ ...segment, q_start: this.numericSmall, D }))
				);
				return {
				...
				DeffMin =
					segment.start_idx === segment.end_idx
						...
						: Math.min(
								...
								Math.max(
									-this.numericLarge,
									arpsD2Deff(arpsGetD({ ...segment, q_end: this.numericLarge }), segment.b)
								)
						  );
				startIdxMin = Math.max(
					this.dateIdxSmall,
					Math.min(segment.end_idx, arpsGetStartIdxFromQstart({ ...segment, D, q_end: this.numericLarge }))
				);
				startIdxMax = segment.end_idx;
				endIdxMin = segment.start_idx;
				endIdxMax = Math.max(
					segment.start_idx,
					Math.min(this.dateIdxLarge, arpsGetEndIdxFromQend({ ...segment, D, q_end: this.numericLarge }))
				);
				return {
					start_idx: [Math.ceil(startIdxMin), Math.floor(startIdxMax)],
					end_idx: [Math.ceil(endIdxMin), Math.floor(endIdxMax)],
					...
					q_start: [
						...
						Math.max(
							this.numericSmall,
							Math.min(this.numericLarge, arpsGetQStart({ ...segment, D, q_end: this.numericLarge }))
						),
					],
			...
			case 'D_eff': {
				...
				const highD = arpsDeff2D(-0.01 - this.numericSmall, segment.b);
				const lowD = arpsDeff2D(-this.numericLarge + this.numericSmall, segment.b);
				...
				startIdxMin = Math.min(
					startEq,
					arpsGetStartIdxFromQstart({ ...segment, D: lowD }),
					Math.max(this.dateIdxSmall, arpsGetStartIdxFromQstart({ ...segment, D: highD }))
				);
				startIdxMax = Math.min(startEq, arpsGetStartIdxFromQstart({ ...segment, D: lowD }));
				endIdxMin = Math.max(endEq, arpsGetEndIdxFromQend({ ...segment, D: lowD }));
				endIdxMax = Math.max(
					endEq,
					arpsGetEndIdxFromQend({ ...segment, D: lowD }),
					Math.min(this.dateIdxLarge, arpsGetEndIdxFromQend({ ...segment, D: highD }))
				);
				return {
					start_idx: [Math.ceil(startIdxMin), Math.floor(startIdxMax)],
					end_idx: [Math.ceil(endIdxMin), Math.floor(endIdxMax)],
					...
					q_start: [
						Math.min(
							...
							Math.max(this.numericSmall, arpsGetQStart({ ...segment, D: lowD }))
						),
						Math.min(segment.q_end, arpsGetQStart({ ...segment, D: highD })),
					],
					q_end: [
						Math.max(segment.q_start, predArps({ ...segment, t: segment.end_idx, D: highD })),
						Math.max(
							segment.q_start,
							predArps({ ...segment, t: segment.end_idx, D: highD }),
							Math.min(this.numericLarge, predArps({ ...segment, t: segment.end_idx, D: lowD }))
						),
					],
	...
	buttonQFinal(qFinalDict, prodInfo, firstSegment) {
		...
		if (!wellLifeValid && !qFinalValid) {
			throw Error(
				`Arps: Target q Final is smaller than the q Start of last segment.` +
