...
import { ApiWellComment, filterableFields, READ_RECORD_LIMIT, sortableFields } from './fields';
import { WellCommentService } from './service';

type Locals = { service: WellCommentService };

const DEFAULT_PAGE_SIZE = 25;

export const getWellCommentsHead = async (req: Request, res: Response<void>): Promise<void> => {
	...
	const count = await service.getWellCommentsCount(filters);
...
export const getWellComments = async (req: Request, res: Response<ApiWellComment[]>): Promise<void> => {
	const { service } = res.locals as Locals;
	const { query } = req;
	...
	const {
		...
		hasNext,
		cursor: cursorNext,
	} = await service.getWellComments(skip, take, sort, filters, cursorQuery);
