...
import {
	ApiDailyProduction,
	ApiDailyProductionKey,
	getApiDailyProductionField,
	getRequiredFields,
...
export const parseApiDailyProduction = (value: Record<string, unknown>, index?: number): ApiDailyProduction => {
	const production: Record<string, ApiDailyProduction[ApiDailyProductionKey]> = {};
	...
	Object.entries(value)
		...
		.forEach(([field, value]) =>
			errorAggregator.catch(() => {
				const wellField = getApiDailyProductionField(field);
				...
				let parsedValue;
				try {
					parsedValue = parse ? parse(value) : (value as ApiDailyProduction[ApiDailyProductionKey]);
...
): Array<ISingleDailyProduction | undefined> => {
			...
			if (occurrences && occurrences.length > 1) {
				...
				throw new DuplicateProductionError(
					`More than one production data supplied for well \`${wellId}\` ` +
						`in \`${moment(date).utc().format('YYYY-MM-DD')}\``,
...
export const parsePostDailyProductions = async (
	...
	let production = apiProduction.map((prod) => prod && toISingleDailyProduction(prod));
	...
	production = production.map((prod, indexInList) =>
		errorAggregator.catch(() => {
			...
			const date = moment(indexToDate(index)).utc().format('YYYY-MM-DD');
...
export const parsePutDailyProductions = async (
	data: unknown[],
	service: DailyProductionService,
	errorAggregator: ValidationErrorAggregator,
	project: BaseProjectResolved | null = null,
...
const mapWellIdToChosenId = (apiProduction: (ApiDailyProduction | undefined)[]) =>
