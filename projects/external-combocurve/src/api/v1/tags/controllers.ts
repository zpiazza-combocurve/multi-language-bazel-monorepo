...
import { ApiTag, filterableFields, READ_RECORD_LIMIT, sortableFields } from './fields/root-tags';
import { TagsService } from './service';

type Locals = { service: TagsService };

const DEFAULT_PAGE_SIZE = 25;

export const getTagsHead = async (req: Request, res: Response<void>): Promise<void> => {
	...
	const count = await service.getTagsCount(filters);
...
export const getTags = async (req: Request, res: Response<ApiTag[]>): Promise<void> => {
	const { service } = res.locals as Locals;
	const { query } = req;
	...
	const { result, hasNext, cursor: cursorNext } = await service.getTags(skip, take, sort, filters, cursorQuery);
