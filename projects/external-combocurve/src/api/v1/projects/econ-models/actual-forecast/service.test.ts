...
import { getRequestFromDocument } from './fields/actual-forecast';
...
const assumptionKey = ACTUAL_FORECAST_KEY;

let connection: Connection;
let service: ActualForecastService;
...
describe('v1/projects/:projectId/econ-models/differentials/service', () => {
	beforeAll(async () => {
		...
		service = new ActualForecastService(context);
	...
	test('getPaginated', async () => {
		...
		await expect(service.getPaginated(0, count + 1, {}, {}, project)).resolves.toStrictEqual({
			result: results.map((r) => getRequestFromDocument(r as IActualOrForecast)),
			hasNext: false,
			cursor: null,
		});
