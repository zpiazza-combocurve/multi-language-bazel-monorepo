...
describe('v1/projects/:projectId/scenarios/:scenarioId/econ-runs/controllers', () => {
	// eslint-disable-next-line jest/expect-expect
	test('getEconRunsHead throws validation error', async () => {
		const { req, res } = mockExpress();

		req.originalUrl = 'projects/5e272bed4b97ed00132f2271/scenarios/5e27305e4f59a9ec64eb576a/econ-runs';
	...
	test('getEconRunsHead runs correctly', async () => {
		...
		const originalUrl = `projects/${project._id}/scenarios/${scenarioId}/econ-runs`;
		...
		count = 51;
		await getEconRunsHead(req, res);
		expect(getEconRunsCount).toHaveBeenLastCalledWith({}, project._id, scenarioId);
	...
	test('getEconRuns throws validation error', async () => {
		...
		req.originalUrl = `projects/${project._id}/scenarios/${scenarioId}/econ-runs`;
	...
	test('getEconRuns runs correctly', async () => {
		...
		expect(serviceEconRun).toHaveBeenLastCalledWith(0, 25, { id: -1 }, {}, project._id, scenarioId, undefined);
		expect(res.set).toHaveBeenLastCalledWith({
		...
		hasNext = true;
		await getEconRuns(req, res);
		expect(serviceEconRun).toHaveBeenLastCalledWith(25, 25, { id: -1 }, {}, project._id, scenarioId, undefined);
		expect(res.set).toHaveBeenLastCalledWith({
			Link:
				`<http://www.localhost.com/${originalUrl}?skip=50&take=25>;rel="next",` +
				`<http://www.localhost.com/${originalUrl}?skip=0&take=25>;rel="prev",` +
		...
		hasNext = false;
		await getEconRuns(req, res);
		expect(serviceEconRun).toHaveBeenLastCalledWith(30, 10, { id: -1 }, {}, project._id, scenarioId, undefined);
		expect(res.set).toHaveBeenLastCalledWith({
			Link:
				`<http://www.localhost.com/${originalUrl}?skip=20&take=10>;rel="prev",` +
				`<http://www.localhost.com/${originalUrl}?skip=0&take=10>;rel="first"`,
		...
		hasNext = false;
		await getEconRuns(req, res);
		expect(serviceEconRun).toHaveBeenLastCalledWith(
			30,
			10,
			{ runDate: -1 },
			{},
		...
		cursor = Types.ObjectId();
		await getEconRuns(req, res);
		expect(serviceEconRun).toHaveBeenLastCalledWith(0, 10, { id: -1 }, {}, project._id, scenarioId, undefined);
		expect(res.set).toHaveBeenLastCalledWith({
			Link:
		...
		cursor = null;
		await getEconRuns(req, res);
		expect(serviceEconRun).toHaveBeenLastCalledWith(0, 10, { runDate: 1 }, {}, project._id, scenarioId, undefined);
		expect(res.set).toHaveBeenLastCalledWith({
			Link:
