...
describe('/v1/projects/{projectId}/econ-models/production-taxes', () => {
	beforeAll(async () => {
	...
	test('Post and get non unique production-taxes', async () => {
		const productionTaxes = createProductionTaxesPayload(productionTaxes_COUNT, { unique: false });

		let response = await app
			.post(`/v1/projects/${project.id}/econ-models/production-taxes`)
		...
		const productionTaxesResponse = await Promise.all(
			productionTaxes.map(
				async (rc) =>
					await app
						.get(`/v1/projects/${project.id}/econ-models/production-taxes?name=${rc.name}`)
						.set(config.headers),
			),
		);
	...
	test('Post and get unique production-taxes', async () => {
		const productionTaxes = createProductionTaxesPayload(productionTaxes_COUNT, {
			unique: true,
	...
	test('HEAD productionTaxes', async () => {
		...
		await testHeadMethod(
			app,
			`/v1/projects/${project.id}/econ-models/production-taxes`,
			config.headers,
	...
	test('DELETE production-taxes', async () => {
		const productionTaxes = createProductionTaxesPayload(1, { unique: false });
		const createResponse = await app
			.post(`/v1/projects/${project.id}/econ-models/production-taxes`)
			.send(productionTaxes)
			.set(config.headers);
		let response = await app.get(`/v1/projects/${project.id}/econ-models/production-taxes`).set(config.headers);
		...
		await testDeleteMethod(
			app,
			`/v1/projects/${project.id}/econ-models/production-taxes/${productionTaxesModelId}`,
			config.headers,
			1,
		);
	});

	test('PUT Upsert production-taxes', async () => {
		const productionTaxes = createProductionTaxesPayload(1, { unique: false });
		const createPutResponse = await app
			.put(`/v1/projects/${project.id}/econ-models/production-taxes`)
			.send(productionTaxes)
			.set(config.headers);
		let getResponse = await app.get(`/v1/projects/${project.id}/econ-models/production-taxes`).set(config.headers);
		...
		getResponse = await app.get(`/v1/projects/${project.id}/econ-models/production-taxes`).set(config.headers);
