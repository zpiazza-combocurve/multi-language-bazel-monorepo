...
describe('/v1/projects/{projectId}/econ-models/general-options', () => {
	let url;
	...
	beforeEach(async () => {
		...
		url = `/v1/projects/${project.id}/econ-models/general-options`;
	});
	...
	function expectActualModel(got, expected) {
		...
		expect(got.mainOptions).toMatchObject(expected.mainOptions);
	}

	test('POST and GET non unique', async () => {
		const models = createGeneralOptionsPayload(2, { unique: false });
	...
	test('POST and GET unique', async () => {
		const models = createGeneralOptionsPayload(2, {
			unique: true,
	...
	test('PUT upsert', async () => {
		const models = createGeneralOptionsPayload(1, { unique: false });
