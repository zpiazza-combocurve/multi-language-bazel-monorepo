...
import { CreateScenarioLookupTableDialog } from '@/lookup-tables/scenario-lookup-table/LookupTable/CreateScenarioLookupTableDialog';
import {
	copyLookupTable,
	deleteLookupTable,
	deleteLookupTables,
...
export default function LookupTableMod() {
	...
	const { canCreate: canCreateScenarioLookupTables } = usePermissions(SUBJECTS.LookupTables, project?._id);
	...
	return (
		<SharedLookupTable
			featureName='Scenario Lookup Tables'
			...
			copyNotification={NotificationType.COPY_LOOKUP_TABLE}
			importNotification={NotificationType.IMPORT_LOOKUP_TABLE}
			getRoutes={(projectId, id) => URLS.project(projectId).scenarioLookupTable(id)}
			createDialog={CreateScenarioLookupTableDialog}
			canCreateLookupTable={canCreateScenarioLookupTables}
			canImportLookupTable={canCreateScenarioLookupTables}
		/>
	);
}
