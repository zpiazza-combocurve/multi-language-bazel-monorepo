...
import { CompletionRowData } from '../types';
...
const getNewCompletionRow = (rows: CompletionRowData[]): CompletionRowData => {
	return {
...
const completionToolbarOperations = (rows): ToolbarOperation[] =>
	getCriteriaBasedNewRowToolbarOperations(rows, () => getNewCompletionRow(rows));

function CompletionForm() {
	const customDateHeaders = useCustomDateHeaders();
	const { watch, setValue } = useFormContext<NodeFormFields<NetworkShared.NodeType.completion>>();
	const watchedRows = watch('time_series.rows');
	const watchedFuelType = watch('time_series.fuel_type');

	const memoizedToolbarOperations = useMemo(() => completionToolbarOperations(watchedRows), [watchedRows]);
	...
	const memoizedColumns = useMemo(
		() =>
			getCompletionColumns(watchedFuelType, {
				customDateHeaders,
			}),
